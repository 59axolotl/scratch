...
const creatorController = {
  ...
  validateAndLogin: async (req, res, next) => {
    try {
      ...
      const isPasswordValid = await bcrypt.compare(req.body.password, req.creatorRecord.password);

      // If creator enters wrong password, return error object to global error handler
      if (!isPasswordValid) {
        return next({
          log: `Invalid creator login attempt, ${err}.`,
          status: 401,
  ...
  logout: async (req, res, next) => {
    ...
    res.locals.logoutMessage = { message: 'You have successfully been logged out.' };
